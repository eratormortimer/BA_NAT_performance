...
local SRC_PORT		= 1234
local DST_PORT		= 319
local ROUNDS        = 10  -- Amount of times we go through the flows TODO good amount of Rounds?
RoundsLoad    = 0       -- variable for loadslave to check in master if enough rounds
RoundsTime    = 0      -- variable for timeSlave to check in master if enough rounds

-- answer ARP requests for this IP on the rx port
-- change this if benchmarking something like a NAT device
local RX_IP		= "200.200.200.200"
-- used to resolve DST_MAC
local GW_IP		= "200.200.200.200"
...
function configure(parser)
    ...
    parser:option("-n --nat", "If testing a Nat give the public IP."):default("10.1.0.10"):convert(tostring)
end
...
function master(args)
    ...
    ------------------------------------------------------------------------
    --here we set these to public ip of nat if we test a nat device
    print(args.nat)
    print(RX_IP)
    RX_IP=args.nat
    GW_IP=args.nat
...
function loadSlave(queue, rxDev, size, flows)
	...
	while mg.running() do
		...
		for i, buf in ipairs(bufs) do
			...
			pkt.ip4.src:set(baseIP + counterSRC)
...
function timerSlave(txQueue, rxQueue, size, flows)
    ...
    local counterBreak = 0
    local baseIP = parseIPAddress(SRC_IP_BASE)
    local baseIPSRC = baseIP
	local rateLimit = timer:new(0.001)
	while mg.running() do
		hist:update(timestamper:measureLatency(size, function(buf)
			fillUdpPacket(buf, size)
			local pkt = buf:getUdpPacket()
			pkt.ip4.src:set(baseIPSRC + counterSRC)
			counterSRC = incAndWrap(counterSRC, flows)
		end))
		rateLimit:wait()
		rateLimit:reset()
