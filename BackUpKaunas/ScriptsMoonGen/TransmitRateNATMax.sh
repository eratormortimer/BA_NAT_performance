...
for j in ${arrayFlow[*]};
    do
    ssh -p 10022 sternsdo@172.16.0.1 "mkdir /srv/testbed/results/manual_test_sternsdo/OVSNATtestMaxPacketsizeFlow$j/"
        for i in ${array[*]};
        do
                cd /root/MoonGen
                echo "starting MoonGen with Rate $i"
                echo "start $(date)" >> output
                echo "$(./build/MoonGen ./examples/l3-load-latency.lua -s 1500 -r $i -f $j 1 0 & sleep 60 ; kill -INT $!)" >> output
                echo "finished with Moongen"
                sleep 20
                echo "end $(date)" >> output
                scp -P 10022 output sternsdo@172.16.0.1:/srv/testbed/results/manual_test_sternsdo/OVSNATtestMaxPacketsizeFlow$j/output-send$n
                scp -P 10022 histogram.csv sternsdo@172.16.0.1:/srv/testbed/results/manual_test_sternsdo/OVSNATtestMaxPacketsizeFlow$j/hist$n.csv
