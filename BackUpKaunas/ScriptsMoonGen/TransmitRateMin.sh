...
arrayFlow=(1 100 1000 10000 100000)
#####
...
for j in ${arrayFlow[*]};
    do
    ssh -p 10022 sternsdo@172.16.0.1 "mkdir /srv/testbed/results/manual_test_sternsdo/OVSNATtestMinimalPacketsizeFlow$j/"
        for i in ${array[*]};
        do
                cd /root/MoonGen
                echo "starting MoonGen with Rate $i"
                echo "start $(date)" >> output
                echo "$(./build/MoonGen ./examples/l3-load-latency.lua -r $i -f $j 0 1 & sleep 60 ; kill -INT $!)" >> output
                echo "finished with Moongen"
                sleep 20
                echo "end $(date)" >> output
                scp -P 10022 output sternsdo@172.16.0.1:/srv/testbed/results/manual_test_sternsdo/OVSNATtestMinimalPacketsizeFlow$j/output-send$n
                scp -P 10022 histogram.csv sternsdo@172.16.0.1:/srv/testbed/results/manual_test_sternsdo/OVSNATtestMinimalPacketsizeFlow$j/hist$n.csv
